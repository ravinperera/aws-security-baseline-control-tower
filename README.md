...
The Python unit tests exercise the repository validator against temporary fixtures for local links, repository-escape attempts, JSON syntax, YAML indentation, AWS account-ID handling, and credential-shape handling. The validator then checks repository JSON syntax, local Markdown links, YAML tab indentation, public reference files for unexpected 12-digit AWS account IDs while permitting the documented `123456789012` placeholder, and a narrow set of high-confidence credential shapes without printing matched values. The credential check covers AWS access-key IDs, GitHub tokens, OpenAI-style API keys, and PEM private-key headers while allowing clearly redacted placeholders. GitHub Actions repeats the tests, repository checks, and Terraform validation in [`.github/workflows/validate.yml`](.github/workflows/validate.yml) using only read access to repository contents.

The credential-shape scan is a regression guard for a public reference repository, not a replacement for organisation-wide secret scanning or repository-hosting secret protection.
